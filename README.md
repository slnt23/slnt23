# Hi there, I'm slnt23 👋

### 👨‍💻 About Me

全栈开发者，专注于 **AI Agent / 后端架构 / 前端工程化**。
Full-stack developer focusing on AI Agents, backend architecture, and frontend engineering.

- 🔭 近期项目: 智能体 AI、价格监测平台、价格信息平台
- 🌱 探索方向: LLM 工程 / MCP 协议 / 系统设计
- 📧 `relax271828@petalmail.com`

### 🛠️ Tech Stack

![Java](https://img.shields.io/badge/Java-ED8B00?logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?logo=springboot&logoColor=white)
![Vue](https://img.shields.io/badge/Vue.js-4FC08D?logo=vuedotjs&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?logo=langchain&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?logo=redis&logoColor=white)
![RabbitMQ](https://img.shields.io/badge/RabbitMQ-FF6600?logo=rabbitmq&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?logo=mysql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black)

### 🔥 Projects

- **健健 Agent** — 基于 LangGraph 的三层调度智能体，支持 MCP 协议工具生态与双轨记忆系统。
- **价格监测与智能分析平台** — SpringBoot + Redis Sentinel 三级缓存，Spring AI 自然语言查价。
- **价格信息平台** — Vue 3 + TypeScript 单仓多角色架构，ECharts 多维价格分析。

### 📫 Connect

[![X](https://img.shields.io/badge/X-000000?logo=x&logoColor=white)](https://x.com/cookorbook)
[![GitHub](https://img.shields.io/badge/GitHub-181717?logo=github&logoColor=white)](https://github.com/slnt23)

![Header Image](resource/header.png)
